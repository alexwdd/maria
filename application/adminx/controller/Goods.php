...
class Goods extends Admin
{
        ...
        if(request()->isPost()){
            ...
            if ($result['code']==1) {
                ...
                if($data['show']==0){
                    ...
                    db("GoodsPush")->where('goodsID',$goods_id)->delete();
                    cache('flash', NULL);
                }
            }
            return $result;
    ...
    public function import(){
        if (request()->isPost()) {
            ...
            for ( $i = 2; $i <= $highestRow; $i++) {
                ...
                $data['cutPrice'] = trim($sheet->getCellByColumnAndRow(9, $i)->getValue());
                $data['inprice'] = trim($sheet->getCellByColumnAndRow(10, $i)->getValue());
                $data['marketPrice'] = trim($sheet->getCellByColumnAndRow(11, $i)->getValue());
                $data['point'] = trim($sheet->getCellByColumnAndRow(12, $i)->getValue());
                $data['sellNumber'] = trim($sheet->getCellByColumnAndRow(13, $i)->getValue());
                $data['weight'] = trim($sheet->getCellByColumnAndRow(14, $i)->getValue());
                $data['wuliuWeight'] = trim($sheet->getCellByColumnAndRow(15, $i)->getValue());
                $data['endDate'] = trim($sheet->getCellByColumnAndRow(16, $i)->getValue());
                $data['number'] = trim($sheet->getCellByColumnAndRow(17, $i)->getValue());           
                $data['show'] = trim($sheet->getCellByColumnAndRow(18, $i)->getValue());
                $data['keyword'] = trim($sheet->getCellByColumnAndRow(19, $i)->getValue());
    ...
    public function export(){
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('L1', '市场价')
            ->setCellValue('M1', '积分')
            ->setCellValue('N1', '初始销量')
            ->setCellValue('O1', '商品重量(kg)')
            ->setCellValue('P1', '物流重量(kg)')
            ->setCellValue('Q1', '保质期')
            ->setCellValue('R1', '单品数量')
        ...
        foreach($list as $k => $v){
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('J'.$num, $v['cutPrice'])
                ->setCellValue('K'.$num, $v['inprice'])
                ->setCellValue('L'.$num, $v['marketPrice'])
                ->setCellValue('M'.$num, $v['point'])
                ->setCellValue('N'.$num, $v['sellNumber'])
                ->setCellValue('O'.$num, $v['weight'])
                ->setCellValue('P'.$num, $v['wuliuWeight'])
                ->setCellValue('Q'.$num, $v['endDate'])
                ->setCellValue('R'.$num, $v['number'])
                ->setCellValue('S'.$num, $v['show'])                
                ->setCellValue('T'.$num, $v['keyword'])
                ->setCellValue('U'.$num, $v['say'])
                ->setCellValue('V'.$num, $v['en']);
        }
