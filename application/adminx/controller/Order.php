	...
	public function info(){
		if(request()->isPost()){
			...
			if (!$list) {
				...
				$goods = db("OrderDetail")->field('*,sum(number) as num')->where("orderID",$list['id'])->group('goodsID')->select(); 
				foreach ($goods as $key => $value) {
	                $item = db('Goods')->where('id='.$value['goodsID'])->find(); 	   
	            }
            	$this->assign('goods',$goods);
		
                $baoguo = db('OrderBaoguo')->where(array('orderID'=>$value['id']))->select();
